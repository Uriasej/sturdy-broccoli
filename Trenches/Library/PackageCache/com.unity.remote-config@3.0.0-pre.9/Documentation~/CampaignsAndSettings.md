...
| `unity.ram` | Amount of RAM in MB on the device. | `unity.ram >= 16384` |
...
To define a Campaign that matches all conditions and always applies, enter `true`.
...
In this example, implement a custom `app` condition to determine what level the player is in.
